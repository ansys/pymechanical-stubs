...
def write_docstring(buffer: typing.TextIO, doc_member: typing.Optional[DocMember], indent_level=1) -> None:
...
def write_enum(buffer: typing.TextIO, enum_type: typing.Any, namespace: str, doc: typing.Dict[str, DocMember], type_filter: typing.Callable=None) -> None:
    logging.debug(f"    writing enum {enum_type.Name}")
    fields = [field for field in enum_type.GetFields() if field.IsLiteral and (type_filter is None or type_filter(field))]
...
def get_properties(class_type: typing.Any, doc: typing.Dict[str, DocMember], type_filter: typing.Callable=None) -> typing.List[Property]:
    ...
    for prop in props:
        ...
        property = Property(getter=False, setter=False, type=prop_type, name=prop_name, doc=prop_doc, static=False, value=None)
        get_method = prop.GetMethod
        if get_method:
            if class_type.IsInterface or get_method.IsPublic:
                property.getter = True
            if get_method.IsStatic: # I don't know how to get the static modifier from the property with reflection
...
def get_methods(class_type: typing.Any, doc: typing.Dict[str, DocMember], type_filter: typing.Callable=None) -> typing.List[Method]:
    # TODO - base class properties are not handled here. They might be published if they are ansys types
    #        or not if they are system types (e.g. the IList methods implemented by an Ansys type that derives from System.Collections.Generic.IList)
    ...
    for method in methods:
        ...
        args = [Param(type=param.ParameterType.ToString(), name=param.Name) for param in params]
        ...
        method = Method(name=method_name, doc=method_doc, return_type=method_return_type, static=method.IsStatic, args=args)
        output.append(method)
    return output


def write_class(buffer: typing.TextIO, class_type:  typing.Any, namespace: str, doc: typing.Dict[str, DocMember], type_filter: typing.Callable=None) -> None:
...
def write_module(namespace: str, mod_types: typing.List, doc: typing.Dict[str, DocMember], outdir: str, type_filter: typing.Callable=None) -> None:
...
def get_namespaces(assembly: "System.Reflection.RuntimeAssembly", type_filter: typing.Callable=None) -> typing.Dict:
    """Get all the namespaces and filtered types in the assembly given by assembly_name."""
    logging.info(f"    Getting types from the {os.path.basename(assembly.CodeBase)} assembly")
